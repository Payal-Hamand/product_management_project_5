...
const updateOrder= async function (req,res) {
    ...
    if (userOrder["cancellable"] == true) {
        if ((isValidStatus(status))) {
            if (userOrder['status'] == 'pending') {
                const updateStatus = await order.findOneAndUpdate({ _id: orderId }, {
                    $set: { status: status }
                }, { new: true })
    ...
    if (userOrder['status'] == "pending") {
        if (status) {
            ...
            const updatedOrderDetails = await order.findOneAndUpdate({ _id: orderId }, { $set: { status: status } }, { new: true })
